...
var meterBk = /Trident.*rv:(\d+\.\d+)/i.test(navigator.userAgent) ? '#EAEAEA' :
                                                                    '#80808040';
...
function drawMeter(c, amount, bk, fg, progress, prog) {
  ...
  ctx.arc(
      c.width / 2, c.height - 58 * sizScale, c.height / 1.8 - ctx.lineWidth,
  ...
  if (typeof progress !== 'undefined') {
    ctx.fillStyle = prog;
    ctx.fillRect(
        c.width * 0.3, c.height - 16 * sizScale, c.width * 0.4 * progress,
        4 * sizScale);
  }
}
...
function updateUI(forced) {
  ...
  drawMeter(
      ...
      mbpsToAmount(Number(UI_DATA.dlStatus * (status == 1 ? oscillate() : 1))),
      meterBk, dlColor, Number(UI_DATA.dlProgress), progColor);
  ...
  drawMeter(
      ...
      mbpsToAmount(Number(UI_DATA.ulStatus * (status == 3 ? oscillate() : 1))),
      meterBk, ulColor, Number(UI_DATA.ulProgress), progColor);
  document.getElementById('pingText').textContent = format(UI_DATA.pingStatus);
  document.getElementById('jitText').textContent = format(UI_DATA.jitterStatus);
}
function oscillate() {
